...
if (player getVariable ["restrained",false])exitWith {["Aktion abgebrochen", "Während du festgenommen bist, kannst du diese Aktion nicht ausführen!", true, "fast"] spawn ESG_fnc_notify;};
...
if (isPlayer _curObject && _curObject isKindOf "Man") then {
    if ((_curObject getVariable ["restrained",false]) && !dialog && playerSide isEqualTo west) then {
		...
		if (!dialog && playerSide isEqualTo independent && (alive _curObject)) then {
