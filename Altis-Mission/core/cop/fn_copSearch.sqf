...
if (count _invs > 0) then {
    ...
    [getPlayerUID _civ, _civ getVariable ["realname",name _civ], "Mitführen von illegalen Gegenständen"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
...
if (!alive _civ || player distance _civ > 5) exitWith {["Durchsuchen fehlgeschlagen",format ["%1 konnte nicht durchsucht werden.",_civ getVariable ["realname",name _civ]], true, "fast"] spawn ESG_fnc_notify;};
//hint format ["%1",_this];
...
["Durchsuchen",format ["<t color='#FF0000'><t size='2'>%1</t></t><br/><t color='#FFD700'><t size='1.5'><br/>" +(localize "STR_Cop_IllegalItems")+ "</t></t><br/>%2<br/><br/><br/><br/><t color='#FF0000'>%3</t>",(_civ getVariable ["realname",name _civ]),_inv,if (_robber) then {"Hat Bank ausgeraubt"} else {""}], false, "fast"] spawn ESG_fnc_notify;
