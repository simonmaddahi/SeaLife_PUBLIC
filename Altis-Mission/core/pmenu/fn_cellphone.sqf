...
_units lbAdd "An den Rettungsdienst";
_units lbSetData [(lbSize _units)-1,"independent"];
_units lbSetColor [(lbSize _units)-1, [0, 1, 0, 0.5]];
/*
_units lbAdd "An den ARC";
_units lbSetData [(lbSize _units)-1,"east"];
_units lbSetColor [(lbSize _units)-1, [1, 1, 0, 0.5]];
*/
...
if((call life_mediclevel) > 4) then {
	_units lbAdd "Rettungsdienst-Meldung";
	_units lbSetData [(lbSize _units)-1,"emsmeldung"];
	_units lbSetColor [(lbSize _units)-1, [0, 1, 0, 0.5]];
};
/*
if((call life_arcLevel) > 4) then {
	_units lbAdd "ARC-Meldung";
	_units lbSetData [(lbSize _units)-1,"arcmeldung"];
	_units lbSetColor [(lbSize _units)-1, [1, 1, 0, 0.5]];
};
*/
_units lbAdd "=========================";
_units lbSetData [(lbSize _units)-1,"strich"];
{
    if (alive _x && _x != player) then {
        switch (side _x) do {
            ...
            case independent: {_type = "Rettungsdient"};
			case east: {_type = "ARC"};
        };
