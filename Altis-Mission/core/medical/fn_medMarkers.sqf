		...
		_text="";
		{
			...
			if((vehicle _unit) isEqualTo _unit) then {
				if(("ItemGPS" in assignedItems _unit)  && !(_unit getVariable["esg_bewusstlos",false]) && !(_x getVariable ["restrained",false])) then {
					...
					_marker setMarkerTextLocal format ["%1 %2", getText(missionConfigFile >> "CfgFMS" >> "medic" >> ((_unit getvariable ["fms",[[], "not"]]) select 1) >> "kurz"),name _unit];
				...
				_text = format ["[%1]", getText(configFile >> "CfgVehicles" >> (typeOf vehicle _unit) >> "displayName")];
				{
					...
					_text = _text + format["%1 %2",getText(missionConfigFile >> "CfgFMS" >> "medic" >> ((_x getvariable ["fms",[[], "not"]]) select 1) >> "kurz"),name _x];
