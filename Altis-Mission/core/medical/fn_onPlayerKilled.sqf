...
[
	"<t size='1' color='#1d3559' align='right'>Warte auf den Rettungsdienst...<br /></t><t size='0.70' color='#FFFFFF' align='right'>Du hast noch eine Chance wiederbelebt zu werden!</t>",
...
if (!isNull _killer && {_killer != _unit} && {side _killer != west} && {alive _killer}) then {
    if (vehicle _killer isKindOf "LandVehicle") then {
        if ((random(100)) < 50) then {
            [getPlayerUID player, (player getVariable ["realname", name player]), "Überfahren einer Person"] remoteExecCall ["life_fnc_wantedAdd", 2];
        };
        ...
        if ((random(100)) < 50) then {
            [getPlayerUID player, (player getVariable ["realname", name player]), "Töten einer Person"] remoteExecCall ["life_fnc_wantedAdd", 2];
        };
...
_unit setVariable ["ESG_DeadGeld",life_cash,true];
{
    _var = configName(_x);
