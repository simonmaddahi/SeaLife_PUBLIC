...
if ((random(100)) < 50) then {
    [getPlayerUID _enemy, (_enemy getVariable ["realname", name _enemy]), "Körperverletzung"] remoteExecCall ["life_fnc_wantedAdd",2];
};
if (isNull _enemy) then {
	[0,format ["%1 hat sich selbst verwundet", player getVariable ["realname", name player]]] remoteExecCall ["life_fnc_broadcast",-2];
} else {
	[0,"STR_MISC_INJURE",true,[player getVariable ["realname",name player], _enemy getVariable ["realname", name _enemy]]] remoteExecCall ["life_fnc_broadcast",-2];
};
...
//systemchat format ["Du wurdest von %1 verwundet!",_enemy getVariable ["realname", name _enemy]];
