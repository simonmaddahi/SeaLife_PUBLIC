...
if (getPlayerUID player isEqualTo (group player getVariable "company_owner")) exitWith {hint localize "STR_GNOTF_LeaderLeave"};
...
if (life_HC_isActive) then {
    [4,group player] remoteExec ["HC_fnc_updateCompany",HC_Life]; //Update the database.
} else {
    [4,group player] remoteExec ["TON_fnc_updateCompany",RSERV]; //Update the database.
};
