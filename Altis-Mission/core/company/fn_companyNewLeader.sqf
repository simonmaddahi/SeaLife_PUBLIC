...
if (_action) then {
    ...
    group player setVariable ["company_owner",_unitID,true];
    group player selectLeader _unit;
    [_unit,group player] remoteExec ["TON_fnc_clientCompanyLeader",_unit]; 

    if (life_HC_isActive) then {
        [3,group player] remoteExec ["HC_fnc_updateCompany",HC_Life]; //Update the database.
    } else {
        [3,group player] remoteExec ["TON_fnc_updateCompany",RSERV]; //Update the database.
    };
