...
if ((ctrlText _aplz isEqualTo "") || {(ctrlText _aplz isEqualTo "PLZ")} || {!([ctrltext _aplz]call TON_fnc_isnumber)} || {(count (ctrlText _aplz) > 5)} || {(count (ctrlText _aplz) < 3)})then {_error pushback _aplz;};
...
if(count _error > 0)exitWith{
  ["Ausweis-Erstellung","Ein paar Sachen sind falsch eingetragen. Überprüfe diese noch einmal!", true, "fast"] spawn ESG_fnc_notify;
  {
