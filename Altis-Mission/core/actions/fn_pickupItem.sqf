...
if ((time - life_action_delay) < 2) exitWith {["Aktion abgebrochen","Du bist ganz aus der Puste! Warte ein paar Sekunden!", true, "fast"] spawn ESG_fnc_notify; INUSE(_this);};
...
if (_diff <= 0) exitWith {["Inventar","Dein Inventar ist voll!", true, "fast"] spawn ESG_fnc_notify; INUSE(_this);};
